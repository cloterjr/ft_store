...
// import { Menu } from '../model/security/menu';
...
export class InMemCatalogService implements InMemoryDbService {
  createDb() {
    ...
    [
      {
        id: 1,
        logo: "assets/images/site/logo_renault_2.png"
      }
    ];
    ...
    //Mock Roles
    let visiter: Menu[] = [];
    visiter.push(menus[0]); // Added Permission to Home

    let client: Menu[] = [];
    client.push(menus[0]); // Added Permission to Home
    client.push(menus[1]); // Added Permission to Catalog

    let admin: Menu[] = [];
    visiter.push(menus[0]); // Added Permission to Home
    visiter.push(menus[1]); // Added Permission to Catalog


    let users: User[] = [
      { id: 1, name: 'Cloter Barbosa', userName: 'cloter', email: 'cloter@ciandt.com', document: '33297056800', password:'123',permissions: client},
      { id: 2, name: 'John Doe', userName: 'jdoe', email: 'jdoe@test.com', document: '12345678909', password:'123', permissions: visiter},
      { id: 3, name: 'Admin', userName: 'admin', email: 'admin@test.com', document: '12345678912', password:'123', permissions: admin}
    ];
